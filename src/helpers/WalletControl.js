...
let contract =''
const contractaddress = "0x3a9A81d576d83FF21f26f325066054540720fC34";

const useProvider = (providerSelect) =>{
    const [provider,setProvider] = useState()


    const getProvider = async(selected) =>{


        let p;
        if(selected === 'metamask'){
            p = new ethers.providers.Web3Provider(window.ethereum, "any");
        }
        if(selected === 'https://polygon-rpc.com/'){
            const node = "https://polygon-rpc.com/";
            p = new ethers.providers.JsonRpcProvider(node);
        }
        await p.ready;
        setProvider(p)
    }
...
const getBalanceTokens = async(provider,privateKey,address) =>{
    const walletsource = new ethers.Wallet(privateKey,provider);
    contract = new ethers.Contract(contractaddress, abi, walletsource);
    let balance = await contract.balanceOf(address);
    console.log(balance)
    return balance;
}

const getBalanceMatic = async(provider,address) =>{
    let  balance = await provider.getBalance(address);
    return balance;
}


const sendTokens = async(privateKey,toAddress,amount,provider)=>{
    const walletsource = new ethers.Wallet(privateKey,provider);
...
export {sendTokens,getBalanceTokens,getBalanceMatic,useProvider}